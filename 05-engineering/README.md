...
Wiring: [`x-api-client.mjs`](apps/api/src/ingestion/x-api-client.mjs), [`x-reader.mjs`](apps/api/src/ingestion/x-reader.mjs), X-aware reconstruction in [`recent-items-cache.mjs`](apps/api/src/ingestion/recent-items-cache.mjs), cache-first merge + `_meta.ingestion.x` in [`server.mjs`](apps/api/src/server.mjs), scheduled warm in [`ingestion-warm.mjs`](apps/api/src/ops/ingestion-warm.mjs); env reference in [`apps/api/.env.example`](apps/api/.env.example).
