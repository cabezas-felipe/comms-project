...
- **Default OFF, now activatable.** Production translation is gated by `TEMPO_TRANSLATION_ENABLED` (default OFF). **Phase 4 S0** wired the production `translateFn` (OpenAI-backed); **Slices 16–18** activated the 6 Spanish feeds (La Silla Vacía, Semana, Infobae) — now `active=true`, `lang=es` in the manifest. Flipping the flag enables ES→EN evidence normalization pre-recall.
