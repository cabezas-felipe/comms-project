...
| # | Topic | Decision |
|---|--------|----------|
| **I1** | On LLM cluster failure | **I1a — Keep** `gracefulFallbackClustering` (deterministic by-topic, caps preserved). |
...
**Providers (reference):** `anthropic:` → Anthropic + timeout; `mock-*` / `TEMPO_AI_MOCK_ONLY` → **`mockCluster`**; unknown → mock-like path. **On throw:** **I1a** fallback.
...
> **Clustering (v1):** `cluster-v1` — LLM path (`clusterItems`) over deduped items + settings; **≤5** stories, **1–5** sources each (schema). On failure → **graceful fallback** (deterministic by topic). **FP-first merge posture:** prefer **not** to merge unrelated items (**I3a**). **Clustering model / provider matrix:** **Chunk N** + code (**I4a**, same deferral idea as **E4a**).
