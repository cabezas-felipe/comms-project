...
> **Reconcile with `server.mjs`:** The “Where you are today” section below was written against an earlier shape (e.g. `buildDashboardPayload`, fixture grouping + `summarizeCluster` on each GET). The API may already implement **persisted snapshots**, `**POST /api/dashboard/refresh`**, and clustering/grounding modules—treat this file as the **requirements + phased roadmap**, and refresh the opening “today” bullets against the current tree when planning the next increment.
...
- **Contract bump:** Extend `storySchema` (or add a parallel field gated by contract version) with `**keywords: string[]`** (and optionally `subtitle`) so UI can render tags without overloading `takeaway`.
...
Add `**clusterItemsWithLlm(items, settings)`** (new module under [apps/api/src/ai/](../apps/api/src/ai/)):
...
- `**meta_story_snapshots`** (example shape): `user_id`, `payload_json` (dashboard-compatible), `generated_at`, `source_watermark` (etag or last item ids).
- Optionally `**raw_items**` for debugging/reprocessing—can defer if snapshot suffices for MVP.

**Routes:**

- `**POST /api/dashboard/bootstrap`** (auth’d): runs ingest + cluster + write snapshot; idempotent enough for retries.
- `**POST /api/dashboard/refresh`:** hourly path; same pipeline but reads previous watermark to avoid duplicates if desired.
- `**GET /api/dashboard`:** read snapshot only (fast); optional `If-None-Match`/etag later.
...
- In [Dashboard.tsx](../../04-prototype/src/pages/Dashboard.tsx):
  - If `bootstrapDashboard`: call `**POST .../bootstrap`**, then `**GET .../dashboard**`.
  - Else (including Settings → Dashboard): **only `GET`**.
- Start `**setInterval` (1h)** calling `**POST .../refresh`** while authenticated session exists or while Dashboard route mounted—pick one consistent behavior and document it (mount-based timer stops when leaving Dashboard unless you move timer to app root).
